...
const handleChange = (
  ...
  setFormData((prev) => ({
    ...prev,
    [name]: type === "checkbox"
      ? (e.target as HTMLInputElement).checked  // ✅ Type narrowed here
      : value,
  }));
};
      ...
      >
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              {[
                { icon: FaEnvelope, name: "email", placeholder: "Email", type: "email", required: true },
                { icon: FaPhoneAlt, name: "telephone", placeholder: "Telephone" },
                { icon: FaFax, name: "fax", placeholder: "Fax" },
                { icon: FaLock, name: "code", placeholder: "Enter Code", required: true },
              ].map(({ icon: Icon, name, placeholder, type = "text", required }) => (
                <div key={name} className="relative">
                  <Icon className="absolute top-3 left-3 text-gray-400" />
                  <input
                    type={type}
